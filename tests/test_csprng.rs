...
fn test_key_uniqueness() {
    ...
    let collision_rate = 0.0; // No collisions in our test
...
fn test_basic_distribution() {
    ...
    // Check that the ratio is close to 50% (should be between 45% and 55%)
...
fn test_nist_preparation() {
    let total_size = 10_000_000; // 10 MB
    ...
    match Csprng::generate_large_random_data(total_size) {
        Ok(data) => {
            ...
            // Write to file for NIST testing
            ...
            // Check for obvious patterns (no long sequences of identical bytes)
            ...
            // Note: File is preserved for NIST STS testing
            // Manual cleanup required after NIST testing: rm nist_test_data.bin
        }
...
fn test_random_bytes_generation() {
    ...
    for &size in &sizes {
        ...
        // Check that data is not all zeros (extremely unlikely but possible)
...
fn test_error_handling() {
    // Test zero size (should probably fail or return empty)
    ...
    // Test very large size (should handle memory allocation)
    let result = Csprng::generate_random_bytes(100_000_000); // 100MB
...
fn test_csprng_integration() {
    // Test that CSPRNG integrates properly with the rest of the system
    let key = Csprng::generate_key().unwrap();
    let iv = Csprng::generate_iv().unwrap();
