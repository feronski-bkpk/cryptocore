...
fn test_hash_stdin() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Stdin hash test output: {}", String::from_utf8_lossy(&output.stderr));

    Ok(())
}

#[test]
fn test_hash_validation_errors() -> Result<(), Box<dyn std::error::Error>> {
    let build_status = Command::new("cargo")
        .args(["build"])
        .status()?;
    ...
    let output = Command::new("./target/debug/cryptocore")
        .args([
            "dgst",
            "--algorithm", "invalid_hash",
            "--input", "somefile.txt",
        ])
        .output()?;

    assert!(!output.status.success(), "Invalid hash algorithm should fail");
    ...
    assert!(!output.status.success(), "Non-existent file should fail");
